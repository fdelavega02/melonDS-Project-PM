...
// The Project PM transport was originally written against Winsock.  Keep the
// wire protocol identical, but provide the small POSIX compatibility layer
// needed by the Linux-native build.
#ifndef _WIN32
using SOCKET = int;
...
static int socketSelectNfds(SOCKET s) { return s + 1; }
#else
static int socketLastError() { return WSAGetLastError(); }
static bool socketWouldBlock(int error) { return error == WSAEWOULDBLOCK; }
static int socketSelectNfds(SOCKET) { return 0; }
#endif
...
struct RelayLink
{
    ...
    bool begin(const sockaddr_in& addr, const char* hello, melonDS::u32 nowMs)
    {
        ...
        else if (socketWouldBlock(socketLastError())) st = RL_CONNECTING;
    ...
    bool pump()
    {
        ...
        if (st == RL_CONNECTING)
        {
            ...
            timeval tv = {0, 0};
            int r = select(socketSelectNfds(s), NULL, &wr, &ex, &tv);
        ...
        if (!out.empty())
        {
            ...
            else if (r == SOCKET_ERROR && !socketWouldBlock(socketLastError())) return false;
        }
        char tmp[8192];
        for (;;)
        {
            ...
            {
                if (!socketWouldBlock(socketLastError())) closed = true;
                break;
            }
        }
...
struct Net
{
...
#else
        if (gethostname(nm, sizeof(nm) - 1) == 0) setStr(myName, nm);
#endif
    }
...
#ifdef _WIN32
        DWORD n = sizeof(nm);
        if (!GetComputerNameA(nm, &n)) { strcpy(nm, "melonDS"); n = 7; }
#else
    ...
    void tick(melonDS::u32 frame)
    {
        ...
        else if (mode == 2 && !peers[0].up)
        {
            ...
            if (connecting)
            {
                ...
                timeval tv = {0,0};
                int r = select(socketSelectNfds(h.s), NULL, &wr, &ex, &tv);
            ...
            else if (frame >= retryAt)
            {
                ...
                else if (socketWouldBlock(socketLastError())) connecting = true;
    ...
    void flush(int i)
    {
        ...
        else if (r == SOCKET_ERROR && !socketWouldBlock(socketLastError())) dropPeer(i);
    }
    ...
    void pumpRecv(int i)
    {
        ...
        for (;;)
        {
            ...
            else { if (!socketWouldBlock(socketLastError())) dropPeer(i); return; }
        }
    }
    ...
    while (emuStatus != emuStatus_Exit)
    {
        ...
        if (emuStatus == emuStatus_Running || emuStatus == emuStatus_FrameStep)
        {
            ...
            if (nframes >= 30)
            {
                ...
                snprintf(melontitle, sizeof(melontitle), "[%d/%.0f] " MELONDS_WINDOW_TITLE, fps, actualfps);
        ...
        else
        {
            ...
            snprintf(melontitle, sizeof(melontitle), MELONDS_WINDOW_TITLE);
            changeWindowTitle(melontitle);
